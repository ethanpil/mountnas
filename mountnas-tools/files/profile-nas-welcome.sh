[ -t 1 ] || return 0
ver=$(cat /usr/share/mountnas/release 2>/dev/null || cat /usr/share/mountnas/version 2>/dev/null || echo "?")
[ -n "$ver" ] && [ "$ver" != dev ] || ver=$(cat /usr/share/mountnas/version 2>/dev/null || echo "?")
