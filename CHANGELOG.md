...
- **`nas ttyd` — browser-based terminal** (off by default, port 22222): `on [port]` / `off` / `status`, with the same commit-honesty warnings as `nas web`. Serves a **real `/bin/login` prompt**, never a bare shell; root is blocked on web terminals by `/etc/securetty` (use a wheel user); plain-HTTP cleartext warning printed on enable. The dashboard footer links to the terminal while it's running.
