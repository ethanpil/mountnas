...
| `nas ttyd` | Browser-based terminal (off by default). `on [port]` enables it (default 22222), `off` disables, `status` reports the URL — same `nas commit` rule and warnings as `nas web`. Serves a **real login prompt** (never a bare shell) over plain HTTP on your trusted LAN — the password transits in cleartext, so treat it like Samba. Root logins are blocked on web terminals (`/etc/securetty` has no pts entries): log in as a wheel user (`adduser <name> wheel`). The dashboard links to it when running. |
...
- **Browser terminal (ttyd)** (off by default): `nas ttyd on && nas commit` — a real login prompt at `http://<box>:22222/` (wheel users only; root is securetty-blocked). Handy when SSH is awkward — a tablet, a borrowed machine, a quick look from the couch.
